 ...
 */
export function refundProductItem(orderNo: string, productId: number) {
  return request<ApiResponse<any>>({
    url: '/orders/refund-product',
    method: 'post',
 ...
 * 获取已退款订单列表
 */
export function getRefundedOrders(params: { page?: number, size?: number }) {
  console.log('调用获取已退款订单列表API, 参数:', params);
  return request<ApiResponse<PageResult<Order>>>({
    url: '/orders/refunded',
    method: 'get',
    params

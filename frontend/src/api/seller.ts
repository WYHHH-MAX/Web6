 ...
 * 获取卖家已退款订单列表
 */
export function getSellerRefundedOrders(params: { page?: number, size?: number }) {
  console.log('请求卖家已退款订单列表，参数:', params);
  ...
  return request<{
    ...
    url: '/seller/orders/refunded',
    method: 'get',
    params
  }).then(response => {
    console.log('卖家已退款订单API响应原始数据:', response);

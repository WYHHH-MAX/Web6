      ...
      >
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'status'">
              <div>
                <a-tag v-if="record.refunded" color="red">Refunded</a-tag>
                <a-tag v-if="record.reviewed" color="green">Reviewed</a-tag>
              </div>
            </template>
          </template>
        </a-table>
...
const fetchOrders = async () => {
  ...
  try {
    ...
    // 如果是退款订单标签页，也使用普通接口，只是传递status=5
    if (activeTab.value !== '0') {
      queryParams.status = activeTab.value;
    }
    
    console.log('获取订单列表，参数:', queryParams);
    response = await getSellerOrders(queryParams);
...
const getPaymentMethodText = (method: string | number | undefined | null) => {
  if (method === undefined || method === null) return 'alipay';
  
  // 确保处理为字符串
  const methodStr = String(method);
  
  // If methodStr is empty after conversion, return alipay
  if (!methodStr.trim()) return 'alipay';

                    ...
                    <div class="review-images" v-if="item.images && item.images.length > 0">
                      <a-image
                        ...
                        :preview="{
                          src: getCommentImageUrl(image),
                          mask: 'Preview',
                          maskClassName: 'review-image-mask'
                        }"
                      />
                    </div>
...
import { getImageUrl, getImageUrls, handleImageError, getCommentImageUrl } from '../utils/imageUtil';
...
:deep(.ant-image-preview-body) {
  z-index: 1001; /* 确保预览在最上层 */
}

:deep(.ant-image-preview-wrap) {
  z-index: 1000; /* 确保预览背景在高层级 */
}
</style> 
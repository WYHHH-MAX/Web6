...
export function getImageUrl(url: string | null | undefined): string {
  ...
  // 特别处理评论图片URL
  if (url.startsWith('/images/comment/')) {
...
/**
 * 特别处理评论图片的URL，确保评论图片正确加载
 * @param url 原始图片URL
 * @returns 处理后的评论图片URL
 */
export function getCommentImageUrl(url: string | null | undefined): string {
  if (!url) {
    return DEFAULT_IMAGE;
  }
...
/**
 * 处理评论图片URL数组
 * @param urls 评论图片URL数组
 * @returns 处理后的评论图片URL数组
 */
export function getCommentImageUrls(urls: string[] | null | undefined): string[] {
  if (!urls || !Array.isArray(urls)) {
    return [];
  }
  return urls.map(url => getCommentImageUrl(url));
}
